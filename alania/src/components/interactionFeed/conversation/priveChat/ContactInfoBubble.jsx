...
import { UserIcon, PhoneIcon, VideoCameraIcon, LockClosedIcon } from '@heroicons/react/24/outline';

function ContactInfoBubble({ contact, currentUser, onViewContactInfo, onClose }) {
  const { username, photoPath } = contact;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        ...
        <h3 className="text-lg font-semibold text-gray-900 text-center truncate">{username || 'Nom inconnu'}</h3>
        <div className="mt-4 flex justify-around">
          ...
          >
            ...
            <span className="text-xs mt-1">Bloquer</span>
          </button>
        </div>
        <button
          onClick={onClose}
